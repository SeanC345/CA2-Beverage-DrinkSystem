...
/**
 * Controller for the Drink Search Results view.
 * Displays search results in a table and provides options to view recipes, delete, or update drinks.
 */
public class DrinkSearchResultsController {

    // FXML elements for the table and its columns
    @FXML
    private TableView<Drink> drinkTable; // Table to display search results
    @FXML
    ...
    /**
     * Updates the table with the current list of drinks from search results.
     */
    public void updateTable() {
        drinkTable.getItems().clear(); // Clear the table before adding new items
        if (App.drinkResults == null || App.drinkResults.size() == 0) { // Check if there are any results
            System.out.println("No drinks available to display."); // Log if no results are found
            return;
        }

        // Add each drink in the search results to the table
